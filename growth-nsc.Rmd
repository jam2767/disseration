...
ann.txt <- data.frame(mean.2010.2001 = 5.9, total.nsc = 72, lab = "*",
                      PFT.y = factor("Mid.Hardwood", levels = c("Early.Conifer","Early.Hardwood","Late.Conifer","Late.Hardwood","Mid.Hardwood")))
...
####Spp plot for only significant species - to be included in Chapter. Larger Figure in Appendix
 ...
 growth.nsc.sig.spp.plot <- ggplot(sig.spp.dat, aes(x = mean.2010.2001, y = total.nsc)) +
  geom_point(alpha = .1) +
  ylim(0,150) +
  xlim(0,5) +
  ylab(expression(paste("Total NSC mg g"^"-1"))) + xlab(expression(paste("2010-2001 Mean Ring Width mm yr"^"-1"))) +
...
growth_nsc_data.10yr$CANOPY15[growth_nsc_data.10yr$CANOPY15 == "2"] <- "Dominant"
  ...
  growth_nsc_data.10yr$CANOPY15 <- factor(growth_nsc_data.10yr$CANOPY15, levels = c("Understory","CoDominant","Dominant"), ordered = TRUE)
...
count.mort.spp <- count(mort.dat.fin$Spp[mort.total.sort])
barplot(count.mort.spp[,2],names.arg=count.mort.spp[,1],las=2, ylab = "Number of Dead Trees", main = "Number of Dead Trees by Species")

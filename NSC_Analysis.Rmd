...
```{r}
#Species Anova
lm.species.total <- lm(total.nsc ~ Spp, data = nsc.adult.total.mass.alive.common)
anova(lm.species.total)
summary(lm.species.total) #R^2 = .5877

ggplot(nsc.adult.total.mass.alive.common, aes(x = Spp, y = total.nsc, fill = wood)) + #not enough colors for spp fill
...
ggplot(nsc.adult.total.mass.alive.common, aes(x = genus, y = total.nsc, fill = "red")) + #not enough colors for genus fill
  geom_boxplot() + theme(axis.text.x = element_text(angle=45,hjust=1,size=12)) +
  theme(axis.text.y = element_text(size=16)) +
  theme(axis.title.x = element_text(size=20)) +
  theme(axis.title.y = element_text(size=20)) +

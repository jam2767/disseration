...
write.csv(check.growth.analyzed, file = "alltrees_growth_analyzed_tracking.csv")
...
ringwidths <- combined.diss[93:142]
...
mean50yr = rowMeans(ringwidths,na.rm=TRUE) #means by individual
column.means = colMeans(ringwidths,na.rm=TRUE) #means by year
us = tapply(X=mean50yr,INDEX=list(s),FUN=mean,na.rm=TRUE)
up = tapply(X=mean50yr,INDEX=list(p),FUN=mean,na.rm=TRUE)
usp = tapply(X=mean50yr,INDEX=list(sp),FUN=mean,na.rm=TRUE)
...
data2 = list(x=as.matrix(combined.diss[,93:142]), nyear=50, 
             nrep = 1415,s=s,p=p,sp=sp,ns=max(s),np = max(p),nsp=max(sp))

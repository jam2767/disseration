...
#removed suspiciously large increment > 1.5 cm cores and removed last year of cores that was > 1.5 cm
combined.diss.clim.common.order <- read.csv("/Users/josh/Dropbox/Dissertation/CH1_Treerings/Data/combined.diss.clim.common.wood.phyl2.CommitteeDraft.QC.csv")
...
###Fix temp and precip data for 1961 - 2010

#Fix increment dataset
  w.i1 <- which(combined.all.diss.common$Site == "W")
  b.i1 <- which(combined.all.diss.common$Site == "B")
  h.i1 <- which(combined.all.diss.common$Site == "H")
  v.i1 <- which(combined.all.diss.common$Site == "V")
  s.i1 <- which(combined.all.diss.common$Site == "S")
  m.i1 <- which(combined.all.diss.common$Site == "M")
  t.i1 <- which(combined.all.diss.common$Site == "T")
  f.i1 <- which(combined.all.diss.common$Site == "F")
  c.i1 <- which(combined.all.diss.common$Site == "C")
  d.i1 <- which(combined.all.diss.common$Site == "D")
  
  #########Correct temperature and precip data for sites used############
  combined.all.diss.common$TEMP_C[w.i1] <- 4.08
  combined.all.diss.common$TEMP_C[b.i1] <- 4.85
  combined.all.diss.common$TEMP_C[h.i1] <- 7.05
  combined.all.diss.common$TEMP_C[v.i1] <- 10.99
  combined.all.diss.common$TEMP_C[s.i1] <- 12.25
  combined.all.diss.common$TEMP_C[m.i1] <- 12.62
  combined.all.diss.common$TEMP_C[t.i1] <- 14.11
  combined.all.diss.common$TEMP_C[f.i1] <- 20.81
  combined.all.diss.common$TEMP_C[c.i1] <- 11.22
  combined.all.diss.common$TEMP_C[d.i1] <- 14.88

  combined.all.diss.common$PRECIP[w.i1] <- 804
  combined.all.diss.common$PRECIP[b.i1] <- 1378
  combined.all.diss.common$PRECIP[h.i1] <- 1135
  combined.all.diss.common$PRECIP[v.i1] <- 985
  combined.all.diss.common$PRECIP[s.i1] <- 970
  combined.all.diss.common$PRECIP[m.i1] <- 1037
  combined.all.diss.common$PRECIP[t.i1] <- 1366
  combined.all.diss.common$PRECIP[f.i1] <- 1298
  combined.all.diss.common$PRECIP[c.i1] <- 2193
  combined.all.diss.common$PRECIP[d.i1] <- 1165
...
#fix mean dataset
  w.m1 <- which(mean.growth.fig.data$Site == "W")
  b.m1 <- which(mean.growth.fig.data$Site == "B")
  h.m1 <- which(mean.growth.fig.data$Site == "H")
  v.m1 <- which(mean.growth.fig.data$Site == "V")
  s.m1 <- which(mean.growth.fig.data$Site == "S")
  m.m1 <- which(mean.growth.fig.data$Site == "M")
  t.m1 <- which(mean.growth.fig.data$Site == "T")
  f.m1 <- which(mean.growth.fig.data$Site == "F")
  c.m1 <- which(mean.growth.fig.data$Site == "C")
  d.m1 <- which(mean.growth.fig.data$Site == "D")
  
  #########Correct temperature and precip data for sites used############
  mean.growth.fig.data$TEMP_C[w.m1] <- 4.08
  mean.growth.fig.data$TEMP_C[b.m1] <- 4.85
  mean.growth.fig.data$TEMP_C[h.m1] <- 7.05
  mean.growth.fig.data$TEMP_C[v.m1] <- 10.99
  mean.growth.fig.data$TEMP_C[s.m1] <- 12.25
  mean.growth.fig.data$TEMP_C[m.m1] <- 12.62
  mean.growth.fig.data$TEMP_C[t.m1] <- 14.11
  mean.growth.fig.data$TEMP_C[f.m1] <- 20.81
  mean.growth.fig.data$TEMP_C[c.m1] <- 11.22
  mean.growth.fig.data$TEMP_C[d.m1] <- 14.88

  mean.growth.fig.data$PRECIP[w.m1] <- 804
  mean.growth.fig.data$PRECIP[b.m1] <- 1378
  mean.growth.fig.data$PRECIP[h.m1] <- 1135
  mean.growth.fig.data$PRECIP[v.m1] <- 985
  mean.growth.fig.data$PRECIP[s.m1] <- 970
  mean.growth.fig.data$PRECIP[m.m1] <- 1037
  mean.growth.fig.data$PRECIP[t.m1] <- 1366
  mean.growth.fig.data$PRECIP[f.m1] <- 1298
  mean.growth.fig.data$PRECIP[c.m1] <- 2193
  mean.growth.fig.data$PRECIP[d.m1] <- 1165
...
mean.growth.fig.data$GenusSpecies <- factor(mean.growth.fig.data$GenusSpecies,
                  levels = c("Juniperus virginiana","Thuja occidentalis","Abies balsamea","Tsuga canadensis","Picea mariana", "Picea rubens","Pinus strobus","Pinus taeda","Pinus palustris","Sassafras albidum","Magnolia virginiana","Liriodendron tulipifera","Oxydendrum arboreum","Nyssa sylvatica","Fraxinus nigra","Fraxinus americana","Acer rubrum","Acer saccharum","Prunus serotina","Ulmus americana","Populus grandidentata","Populus tremuloides","Carya glabra","Carya tomentosa","Betula lenta","Betula alleghaniensis","Betula papyrifera","Fagus grandifolia","Quercus alba","Quercus laevis","Quercus velutina","Quercus rubra","Quercus laurifolia","Quercus coccinea","Quercus nigra","Quercus montana","Quercus muehlenbergii"))

#as factor: PLOT
mean.growth.fig.data$PLOT <- factor(mean.growth.fig.data$PLOT)
combined.all.diss.common$PLOT <- factor(combined.all.diss.common$PLOT) 

#as factor: canopy average
mean.growth.fig.data$CANOPY15 <- as.character( mean.growth.fig.data$CANOPY15)
mean.growth.fig.data$CANOPY15[ mean.growth.fig.data$CANOPY15 == "0"] <- "Understory"
mean.growth.fig.data$CANOPY15[ mean.growth.fig.data$CANOPY15 == "1"] <- "CoDominant"
mean.growth.fig.data$CANOPY15[ mean.growth.fig.data$CANOPY15 == "2"] <- "Dominant"

mean.growth.fig.data$CANOPY15 <- factor(mean.growth.fig.data$CANOPY15, levels = c("Understory","CoDominant","Dominant"))

write.csv(mean.growth.fig.data, "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Data/mean.growth.fig.data.csv")

#as factor: canopy increment
combined.all.diss.common$CANOPY15 <- as.character( combined.all.diss.common$CANOPY15)
combined.all.diss.common$CANOPY15[ combined.all.diss.common$CANOPY15 == "0"] <- "Understory"
combined.all.diss.common$CANOPY15[ combined.all.diss.common$CANOPY15 == "1"] <- "CoDominant"
combined.all.diss.common$CANOPY15[ combined.all.diss.common$CANOPY15 == "2"] <- "Dominant"

combined.all.diss.common$CANOPY15 <- factor(combined.all.diss.common$CANOPY15, levels = c("Understory","CoDominant","Dominant"))

#limiting range of spp boxplot for visualization
litu2 <- which(mean.growth.fig.data$SPP == "LITU")
max(mean.growth.fig.data$mean.growth.50yr[litu2])   ### max is 5.629344 mm/yr
...
growth.spp.plot <- ggplot(mean.growth.fig.data, aes(x = GenusSpecies, y = mean.growth.50yr, fill = PFT)) +
  ...
  annotate("text", x=12, y=4.8,label = "*", size = 5) + #point moved into frame dbh = 40.1 mean growth = 7.42
  theme(axis.title.y = element_text(size=14)) +
  theme(legend.text = element_text(size=10)) +
  theme(legend.title = element_text(size = 14)) +
  theme(legend.key.size = unit(.4, "cm")) +
  labs(fill = "PFT") +
  stat_summary(fun.data = give.n, geom= "text", size = 2.5, aes(y= 5.2)) +
...
#Associated Phylogenetic Tree is phylogenetic_tree_50yr_final_draft.png

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/Growth.AllCommonSpp.png", plot = growth.spp.plot, dpi = 600, width = 10, height = 5.5, units = "in")

#year
combined.all.diss.common$YEAR <- factor(combined.all.diss.common$YEAR, ordered = FALSE)
...
boxplot(INCREMENT ~ YEAR, data = combined.all.diss.common)

lmer.yr <- lmer(INCREMENT ~ YEAR + (1|YEAR), data = combined.all.diss.common)
anova(lmer.yr)
summary(lmer.yr)

lmer.yr.R2 <- r.squaredGLMM(lmer.yr)
fixed.yr <- lmer.yr.R2[1] #.35
rand.yr <- lmer.yr.R2[2] - lmer.yr.R2[1] #.37


##growth by year - mean with sd
year.growth.sd <- ggplot(combined.all.diss.common, aes(x = as.factor(YEAR), y = INCREMENT)) +
...
ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/annual.growth.sd.redo.png", plot = year.growth.sd, dpi = 600, width = 6, height = 3.5, units = "in")

#Individuals as factor
combined.all.diss.common$TAG <- factor(combined.all.diss.common$TAG)
combined.all.diss.common$CORE_ID15 <- factor(combined.all.diss.common$CORE_ID15) #use this for INDIVIDUALS

#Relevel sites
mean.growth.fig.data$SITE <- factor(mean.growth.fig.data$SITE, levels = c("W","B","H","V","C","S","M","T","D","F"))

#lm site
lm.site.avg.final <- lm(mean.growth.50yr ~ SITE, data = mean.growth.fig.data)
anova.site.avg <- anova(lm.site.avg.final)
summary(lm.site.avg.final)

summary(aov.site <- aov(lm.site.avg.final))
...
site.growth.violin <- ggplot(mean.growth.fig.data, aes(x = SITE, y = mean.growth.50yr))+
  ...
  stat_summary(fun.data = give.n, geom= "text", size = 3.5,aes(y= 5.2)) + 
  stat_summary(fun.y="median", geom="point") +
  coord_cartesian(ylim = c(0,5.25)) +
  ylab(expression(paste("Ring Width mm yr"^"-1"))) + xlab("Site") +
  scale_x_discrete(labels = c("W","B","H","V","C","S","M","T","D","F")) +
  scale_fill_manual(values=rev(cbPalette)) +
  annotate("text", x = c(1.25, 2.2, 3.2, 4.3, 5.2, 6.2, 7.25, 8.3, 9.35, 10.2), y = c(3.2, 4.6, 3.55, 3.45, 4.9, 4.9, 3.85, 3.8, 4.2, 4.9), label = c("ce","e","d","cde","e","b","bd","bcd","bcde","a"))

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/site.growth.violin.png", plot = site.growth.violin, dpi = 600, width = 7.5, height = 3.5, units = "in")

#space AVERAGE growth
lm.space.avg.final <- lm(mean.growth.50yr ~ SITE/PLOT/SUB, data = mean.growth.fig.data)
anova.space.avg <- anova(lm.space.avg.final)
summary(lm.space.avg.final)

#variances
avg.space.ss <- anova.space.avg$`Sum Sq`/sum(anova.space.avg$`Sum Sq`)
avg.space.part.vars <- cbind(rownames(anova.space.avg), anova.space.avg$Df, anova.space.avg$`F value`, anova.space.avg$`Pr(>F)`, avg.space.ss)


#variances AVERAGE growth
space.avg.ss <- anova.space.avg$`Sum Sq`/sum(anova.space.avg$`Sum Sq`)
space.avg.part.vars <- cbind(rownames(anova.space.avg), anova.space.avg$Df, anova.space.avg$`F value`, anova.space.avg$`Pr(>F)`, space.avg.ss)

#Temperature AVERAGE
lm.tmp.avg.final <- lm(mean.growth.50yr ~ TEMP_C, data = mean.growth.fig.data)
anova.tmp.avg <- anova(lm.tmp.avg.final)
summary(lm.tmp.avg.final)

#temperature variances AVERAGE growth
temp.avg.ss <- anova.tmp.avg$`Sum Sq`/sum(anova.tmp.avg$`Sum Sq`)
temp.avg.part.vars <- cbind(rownames(anova.tmp.avg), anova.tmp.avg$Df, anova.tmp.avg$`F value`, anova.tmp.avg$`Pr(>F)`, temp.avg.ss)

#Precip AVERAGE
lm.precip.avg.final <- lm(mean.growth.50yr ~ PRECIP, data = mean.growth.fig.data)
anova.precip.avg <- anova(lm.precip.avg.final)
summary(lm.precip.avg.final)

#Precipitation variances AVERAGE growth
precip.avg.ss <- anova.precip.avg$`Sum Sq`/sum(anova.precip.avg$`Sum Sq`)
precip.avg.part.vars <- cbind(rownames(anova.precip.avg), anova.precip.avg$Df, anova.precip.avg$`F value`, anova.precip.avg$`Pr(>F)`, precip.avg.ss)

#DBH AVERAGE
lm.dbh.avg.final <- lm(mean.growth.50yr ~ DBH15, data = mean.growth.fig.data)
anova.dbh.avg <- anova(lm.dbh.avg.final)
summary(lm.dbh.avg.final)

#DBH variances AVERAGE growth
dbh.avg.ss <- anova.dbh.avg$`Sum Sq`/sum(anova.dbh.avg$`Sum Sq`)
dbh.avg.part.vars <- cbind(rownames(anova.dbh.avg), anova.dbh.avg$Df, anova.dbh.avg$`F value`, anova.dbh.avg$`Pr(>F)`, dbh.avg.ss)

#DBH AVERAGE growth plot
dbh.growth.plot.avg <- ggplot(mean.growth.fig.data, aes(x = DBH15, y = mean.growth.50yr)) +
  geom_point(alpha = .3) +
  coord_cartesian(ylim = c(0,6)) +
  geom_smooth(method = "lm", aes(color = "yellow")) +
  theme(axis.title.y = element_text(size = 14)) +
  theme(axis.title.x = element_text(size = 14)) +
  annotate("text",x=40.1,y=5.9,label = "*", size = 6) + #point moved into frame dbh = 40.1 mean growth = 7.42
  scale_color_manual(name = "Fit", values = "yellow", labels = "linear") + 
  theme(legend.position = "none") +
  ylab(expression(paste("Mean Ring Width mm yr"^"-1"))) + xlab("DBH (cm)")

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/DBH.growth.png", plot = dbh.growth.plot.avg, dpi = 600, width = 5, height = 3.5, units = "in")

#Canopy Position AVERAGE
lm.can.avg.final <- lm(mean.growth.50yr ~ CANOPY15, data = mean.growth.fig.data)
anova.can.avg <- anova(lm.can.avg.final)
summary(lm.can.avg.final)

#Canopy Position variances AVERAGE growth
can.avg.ss <- anova.can.avg$`Sum Sq`/sum(anova.can.avg$`Sum Sq`)
can.avg.part.vars <- cbind(rownames(anova.can.avg), anova.can.avg$Df, anova.can.avg$`F value`, anova.can.avg$`Pr(>F)`, can.avg.ss)

#TukeyHSD canopy
summary(aov.canopy <- aov(lm.can.avg.final))
...
canopy.growth.violin <- ggplot(subset(mean.growth.fig.data, !is.na(CANOPY15)), aes(x = CANOPY15, y = mean.growth.50yr, fill = NULL)) +
  geom_violin() + theme(axis.text.x = element_text(angle=45,hjust=1,size=16)) +
  ...
  stat_summary(fun.data = give.n, geom= "text", size = 3.5, aes(y= 5.25)) + 
  stat_summary(fun.y="median", geom="point") +
  ylab(expression(paste("Ring Width mm yr"^"-1"))) + xlab("Canopy Status") + #ggtitle("All Wood Types") +
  ylim(0,5.25) +
  scale_x_discrete(labels = c("Understory", "CoDominant", "Dominant")) +
  scale_fill_manual(values=rev(cbPalette)) +
  annotate("text", x = 1.1:3.1, y = c(3.6, 4.9, 4.9), label = c("a","b","c"), size = 3.5)

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/canopy.growth.violin.png", plot = canopy.growth.violin, dpi = 600, width = 5, height = 3.5, units = "in")


#PFT AVERAGE
lm.pft.avg.final <- lm(mean.growth.50yr ~ PFT, data = mean.growth.fig.data)
anova.pft.avg <- anova(lm.pft.avg.final)
summary(lm.pft.avg.final)

#PFT variances AVERAGE growth
pft.avg.ss <- anova.pft.avg$`Sum Sq`/sum(anova.pft.avg$`Sum Sq`)
pft.avg.part.vars <- cbind(rownames(anova.pft.avg), anova.pft.avg$Df, anova.pft.avg$`F value`, anova.pft.avg$`Pr(>F)`, pft.avg.ss)

#Tukey's HSD
summary(aov.pft <- aov(lm.pft.avg.final))
...
pft.growth.violin <- ggplot(mean.growth.fig.data, aes(x = PFT, y = mean.growth.50yr, fill = PFT)) +
  geom_violin() + theme(axis.text.x = element_text(angle=45,hjust=1,size=16)) +
  theme(axis.text.y = element_text(size=12)) +
  theme(axis.title.x = element_text(size=14)) +
  theme(axis.title.y = element_text(size=14)) +
  ...
  scale_fill_manual(values=rev(cbPalette)) +
  annotate("text", x = 1.1:5.1, y = c(4.9, 2.8, 3.9, 4.9, 2.8), label = c("c","a","b","b","a"))

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/pft.growth.violin.png", plot = pft.growth.violin, dpi = 600, width = 7, height = 3.5, units = "in")


#Species AVERAGE
#### Need to add growth rate by species plot just like nsc-growth by spp plot in chapter 3
#### If keeping this comparison should probably set ACRU as reference species
lm.spp.avg.final <- lm(mean.growth.50yr ~ SPP, data = mean.growth.fig.data)
anova.spp.avg <- anova(lm.spp.avg.final)
summary(lm.spp.avg.final)

#Species variances AVERAGE growth
spp.avg.ss <- anova.spp.avg$`Sum Sq`/sum(anova.spp.avg$`Sum Sq`)
spp.avg.part.vars <- cbind(rownames(anova.spp.avg), anova.spp.avg$Df, anova.spp.avg$`F value`, anova.spp.avg$`Pr(>F)`, spp.avg.ss)

#Wood Anatomy AVERAGE
mean.growth.fig.data$WOOD <- factor(mean.growth.fig.data$WOOD, 
                                    levels = c("large.resin","small.resin","no.resin",
                                               "ring.porous","semiring.porous",
                                               "diffuse.porous"))

#Check to see what I want as the reference group
lm.wood.avg.final <- lm(mean.growth.50yr ~ WOOD, data = mean.growth.fig.data)
anova.wood.avg <- anova(lm.wood.avg.final)
summary(lm.wood.avg.final)

###AIC for wood, pft and spp
aic <- AIC(lm.wood.avg.final, lm.spp.avg.final, lm.pft.avg.final)

#Wood Anatomy variances AVERAGE growth
wood.avg.ss <- anova.wood.avg$`Sum Sq`/sum(anova.wood.avg$`Sum Sq`)
wood.avg.part.vars <- cbind(rownames(anova.wood.avg), anova.wood.avg$Df, anova.wood.avg$`F value`, anova.wood.avg$`Pr(>F)`, wood.avg.ss)

#TukeyHSD WOOD
summary(aov.wood <- aov(lm.wood.avg.final))
...
#limiting range of wood violinplot for visualization
ringpor <- which(mean.growth.fig.data$WOOD == "ring.porous")
difpor <- which(mean.growth.fig.data$WOOD == "diffuse.porous")

max(mean.growth.fig.data$mean.growth.50yr[ringpor])   ### max is 7.420526 mm/yr
max(mean.growth.fig.data$mean.growth.50yr[difpor])   ### max is 5.629344 mm/yr


wood.growth.violin <- ggplot(mean.growth.fig.data, aes(x = WOOD, y = mean.growth.50yr, fill = NULL))+
  geom_violin() + theme(axis.text.x = element_text(angle=45,hjust=1,size=16)) +
  theme(axis.text.y = element_text(size=12)) +
  theme(axis.title.x = element_text(size=14)) +
  theme(axis.title.y = element_text(size=14)) +
  ...
  theme(legend.text = element_blank()) +
  theme(legend.position = "none") +
  stat_summary_bin(fun.data = give.n, geom= "text", size = 3.5, aes(y= 5.2)) +
  stat_summary(fun.y="median", geom="point") +
  ...
  scale_fill_manual(values=rev(cbPalette)) +
  coord_cartesian(ylim = c(0,5.25)) +
  annotate("text", x = 1.15:6.15, y = c(4.9, 2.4, 3.6, 4.9, 2.9, 4.9), label = c("c","a","ab","b","ab","a"))

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/growth.wood.anat.png", plot = wood.growth.violin, dpi = 600, width = 5, height = 3.5, units = "in")


## Interactions for AVERAGE growth ##
#Find that pft and species are responding differently to temperature and overall site effect
#Wood type categories: Hardwood, Conifer
mean.growth.fig.data$PFT <- as.character(mean.growth.fig.data$PFT)

  hardwood <- which(mean.growth.fig.data$PFT == "Early.Hardwood")
  hardwood2 <- which(mean.growth.fig.data$PFT == "Mid.Hardwood")
  hardwood3 <- which(mean.growth.fig.data$PFT == "Late.Hardwood")
                      
  conifer <- which(mean.growth.fig.data$PFT == "Early.Conifer")
  conifer2 <- which(mean.growth.fig.data$PFT == "Late.Conifer")
  
  mean.growth.fig.data$woodgroup <- NA
  mean.growth.fig.data$woodgroup[hardwood] <- "Hardwood"
  mean.growth.fig.data$woodgroup[hardwood2] <- "Hardwood"
  mean.growth.fig.data$woodgroup[hardwood3] <- "Hardwood"
  mean.growth.fig.data$woodgroup[conifer] <- "Conifer"
  mean.growth.fig.data$woodgroup[conifer2] <- "Conifer"
  
    #Hardwood as reference
mean.growth.fig.data$woodgroup <- factor(mean.growth.fig.data$woodgroup, 
                                    levels = c("Hardwood","Conifer"),
                                    ordered = FALSE)

  #Set succession
  mean.growth.fig.data$succession <- NA
  mean.growth.fig.data$succession[hardwood] <- "Early"
  mean.growth.fig.data$succession[conifer] <- "Early"
  mean.growth.fig.data$succession[hardwood2] <- "Mid"
  mean.growth.fig.data$succession[hardwood3] <- "Late"
  mean.growth.fig.data$succession[conifer2] <- "Late"
  
  #Early as reference
mean.growth.fig.data$succession <- factor(mean.growth.fig.data$succession, 
                            levels = c("Early","Mid","Late"),
                            ordered = FALSE)

# #create dataframe from labeling the outlier on mid hardwood
# ann.txt <- data.frame(mean.growth.50yr = 4.9, TEMP_C = 10, lab = " ",
#                       succession = factor("Mid", levels = c("Early","Mid","Late")))

# PFT * Temperature
lm.pft.temp.avg <- lm(mean.growth.50yr ~ PFT*TEMP_C, data = mean.growth.fig.data)
anova.pft.tmp.avg <- anova(lm.pft.temp.avg)
summary(lm.pft.temp.avg)

#PFT * Temperature variances AVERAGE growth
pft.temp.ss <- anova.pft.tmp.avg$`Sum Sq`/sum(anova.pft.tmp.avg$`Sum Sq`)
pft.temp.part.vars <- cbind(rownames(anova.pft.tmp.avg), anova.pft.tmp.avg$Df, anova.pft.tmp.avg$`F value`, anova.pft.tmp.avg$`Pr(>F)`, pft.temp.ss)

### regressions that match the plots now ###
early.con <- which(mean.growth.fig.data$PFT == "Early.Conifer")
late.con <- which(mean.growth.fig.data$PFT == "Late.Conifer")
early.hard <- which(mean.growth.fig.data$PFT == "Early.Hardwood")
mid.hard <- which(mean.growth.fig.data$PFT == "Mid.Hardwood")
late.hard <- which(mean.growth.fig.data$PFT == "Late.Hardwood")

### Temperature ###
lm.pft.tmp.early.con <- lm(mean.growth.50yr[early.con] ~ TEMP_C[early.con], data = mean.growth.fig.data)
summary(lm.pft.tmp.early.con)

lm.pft.tmp.late.con <- lm(mean.growth.50yr[late.con] ~ TEMP_C[late.con], data = mean.growth.fig.data)
summary(lm.pft.tmp.late.con)

lm.pft.tmp.early.hard <- lm(mean.growth.50yr[early.hard] ~ TEMP_C[early.hard], data = mean.growth.fig.data)
summary(lm.pft.tmp.early.hard)

lm.pft.tmp.mid.hard <- lm(mean.growth.50yr[mid.hard] ~ TEMP_C[mid.hard], data = mean.growth.fig.data)
summary(lm.pft.tmp.mid.hard)

lm.pft.tmp.late.hard <- lm(mean.growth.50yr[late.hard] ~ TEMP_C[late.hard], data = mean.growth.fig.data)
summary(lm.pft.tmp.late.hard)


###ggplot graph of each pft by temperature
pft.growth.temp.plot <- ggplot(mean.growth.fig.data, aes(x = TEMP_C, y = mean.growth.50yr)) +
  geom_point(alpha = .1) +
...
ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/pft.growth.temp.plot.png", plot = pft.growth.temp.plot, dpi = 600, width = 5, height = 3.5, units = "in")


# PFT * Precip
lm.pft.precip.avg <- lm(mean.growth.50yr ~ PFT*PRECIP, data = mean.growth.fig.data)
anova.pft.precip.avg <- anova(lm.pft.precip.avg)
summary(lm.pft.precip.avg)

#PFT * Precip variances AVERAGE growth
pft.precip.ss <- anova.pft.precip.avg$`Sum Sq`/sum(anova.pft.precip.avg$`Sum Sq`)
pft.precip.part.vars <- cbind(rownames(anova.pft.precip.avg), anova.pft.precip.avg$Df, anova.pft.precip.avg$`F value`, anova.pft.precip.avg$`Pr(>F)`, pft.precip.ss)

lm.pft.precip.early.con <- lm(mean.growth.50yr[early.con] ~ PRECIP[early.con], data = mean.growth.fig.data)
summary(lm.pft.precip.early.con)

lm.pft.precip.late.con <- lm(mean.growth.50yr[late.con] ~ PRECIP[late.con], data = mean.growth.fig.data)
summary(lm.pft.precip.late.con)

lm.pft.precip.early.hard <- lm(mean.growth.50yr[early.hard] ~ PRECIP[early.hard], data = mean.growth.fig.data)
summary(lm.pft.precip.early.hard)

lm.pft.precip.mid.hard <- lm(mean.growth.50yr[mid.hard] ~ PRECIP[mid.hard], data = mean.growth.fig.data)
summary(lm.pft.precip.mid.hard)

lm.pft.precip.late.hard <- lm(mean.growth.50yr[late.hard] ~ PRECIP[late.hard], data = mean.growth.fig.data)
summary(lm.pft.precip.late.hard)

###ggplot graph of each pft by precipitation
pft.growth.precip.plot <- ggplot(mean.growth.fig.data, aes(x = PRECIP, y = mean.growth.50yr)) +
  geom_point(alpha = .1) +
  coord_cartesian(ylim = c(0,5.2)) +
...
ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/pft.growth.precip.plot.png", plot = pft.growth.precip.plot, dpi = 600, width = 5, height = 3.5, units = "in")


# SPP * Temperature
lm.spp.temp.avg <- lm(mean.growth.50yr ~ GenusSpecies*TEMP_C, data = mean.growth.fig.data)
anova.spp.tmp.avg <- anova(lm.spp.temp.avg)
summary(lm.spp.temp.avg)

#Spp * Temperature variances AVERAGE growth
spp.temp.ss <- anova.spp.tmp.avg$`Sum Sq`/sum(anova.spp.tmp.avg$`Sum Sq`)
spp.temp.part.vars <- cbind(rownames(anova.spp.tmp.avg), anova.spp.tmp.avg$Df, anova.spp.tmp.avg$`F value`, anova.spp.tmp.avg$`Pr(>F)`, spp.temp.ss)

spp.growth.temp.plot <- ggplot(mean.growth.fig.data, aes(x = TEMP_C, y = mean.growth.50yr)) +
  geom_point(alpha = .1) +
  coord_cartesian(ylim = c(0,5.2)) +
  ylab(expression(paste("Ring Width mm yr"^"-1"))) + xlab("Temperature (C)") +
  geom_smooth(method = "lm") + facet_wrap(~ GenusSpecies) +
  theme(axis.title.y = element_text(size = 12)) +
  theme(strip.text.x = element_text(size=7)) +
  theme(axis.title.x = element_text(size = 12)) 

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/spp.growth.temp.plot.png", plot = spp.growth.temp.plot, dpi = 600, width = 7, height = 9, units = "in")

# SPP * Precip
lm.spp.precip.avg <- lm(mean.growth.50yr ~ GenusSpecies*PRECIP, data = mean.growth.fig.data)
anova.spp.precip.avg <- anova(lm.spp.precip.avg)
summary(lm.spp.precip.avg)

#Spp * Precip variances AVERAGE growth
spp.precip.ss <- anova.spp.precip.avg$`Sum Sq`/sum(anova.spp.precip.avg$`Sum Sq`)
spp.precip.part.vars <- cbind(rownames(anova.spp.precip.avg), anova.spp.precip.avg$Df, anova.spp.precip.avg$`F value`, anova.spp.precip.avg$`Pr(>F)`, spp.precip.ss)

spp.growth.precip.plot <- ggplot(mean.growth.fig.data, aes(x = PRECIP, y = mean.growth.50yr)) +
  geom_point(alpha = .1) +
  coord_cartesian(ylim = c(0,5.2)) +
  ylab(expression(paste("Ring Width mm yr"^"-1"))) + xlab("Precipitation (mm)") +
  geom_smooth(method = "lm") + facet_wrap(~ GenusSpecies) +
  theme(axis.title.y = element_text(size = 12)) +
  theme(strip.text.x = element_text(size=7)) +
  theme(axis.title.x = element_text(size = 12)) 

ggsave(filename = "/Users/josh/Dropbox/Dissertation/CH1_Treerings/Figures/Final/spp.growth.precip.plot.png", plot = spp.growth.precip.plot, dpi = 600, width = 7, height = 9, units = "in")

#AIC for climate interactions
AIC(lm.pft.temp.avg, lm.pft.precip.avg, lm.spp.temp.avg, lm.spp.precip.avg)

#PFT * Site
lm.pft.site.avg <- lm(mean.growth.50yr ~ PFT*SITE, data = mean.growth.fig.data)
anova.pft.site.avg <- anova(lm.pft.site.avg)
summary(lm.pft.site.avg)

#SPP * Site
lm.spp.site.avg <- lm(mean.growth.50yr ~ SPP*SITE, data = mean.growth.fig.data)
anova.spp.site.avg <- anova(lm.spp.site.avg)
summary(lm.spp.site.avg)

### Full model for average growth ###
lmer.full.avg <- lmer(mean.growth.50yr ~ PFT + DBH15 + CANOPY15 + (1|SITE/PLOT/SUB) + (1|GenusSpecies) + (1|GenusSpecies:SITE) + (1|PFT:SITE), data = mean.growth.fig.data)
anova.full.avg <- anova(lmer.full.avg)
summary(lmer.full.avg)

full.R2.avg <- r.squaredGLMM(lmer.full.avg)
full.fix.avg <- full.R2.avg[1] #.123
full.rand.avg <- full.R2.avg[2] - full.R2.avg[1] #.477

#proportion of variance for fixed effects from Sum Sq of ANOVA table
full.fix.ss <- anova.full.anual$`Sum Sq`/sum(anova.full.anual$`Sum Sq`)
full.fix.vars.table <- cbind(rownames(anova.full.anual), anova.full.anual$Df, anova.full.anual$`F value`, anova.full.anual$`Pr(>F)`, full.fix.ss)
proportion.fix <- full.fix * as.numeric(full.fix.vars.table[,4]) #proportion of fixed effect explained by each variable - highly dependent on order - don't trust relative importance.
full.fix.table <- cbind(rownames(anova.full.anual),proportion.fix) #final table fixed effects
#random vars
full.rand.vars <- as.data.frame(VarCorr(lmer.full.annual, order = "cov.last")) #random effect variances
prop.full.rand.vars = full.rand.vars$vcov[1:9]/sum(full.rand.vars$vcov[1:9])
#percent.full.rand.vars <- prop.nsc.full.rand.vars*100
proportion.rand <- full.rand * prop.full.rand.vars #proportion of variance explained by each random effect
full.rand.table <- cbind(full.rand.vars$grp[1:9], proportion.rand) #final table random effects
full.residual <- 1 - full.R2[2] #residual
sum(proportion.rand) #check sums
sum(proportion.fix) #check sums


####################### Switching to ANNUAL growth ###########################
#Final Full model from Draft 3
...
# #annual spatial model alone
 lmer.space.only.annual <- lmer(INCREMENT ~ (1|SITE/PLOT/SUB/CORE_ID15), data = combined.all.diss.common)
 anova.space.only.annual <- anova(lmer.space.only.annual)
 summary(lmer.space.only.annual)
 
 #variances for space only
 space.only.vars <- as.data.frame(VarCorr(lmer.space.only.annual, order = "cov.last")) #random effect variances
 var.space.only.vars = space.only.vars$vcov/sum(space.only.vars$vcov)
 percent.var.only.space <- var.space.only.vars*100

#### Going to just run the full model and see what happens ###
 ###############Final model for dissertation chapter #####################
combined.all.diss.common$YEAR <- as.numeric(combined.all.diss.common$YEAR)
 
 #add year as fixed effect for full model in main text. This model is in appendix
lmer.full.annual <- lmer(INCREMENT ~ PFT + WOOD + CANOPY15 + DBH15 + (1|SITE/PLOT/SUB/CORE_ID15) + (1|GenusSpecies) + (1|YEAR) + (1|SITE:YEAR) + (1|GenusSpecies:SITE) + (1|PFT:SITE), data = combined.all.diss.common)
summary(lmer.full.annual)
anova.full.anual <- anova(lmer.full.annual)

#R2m - fixed effects, .1228853
#R2c - random effects + fixed effects, .6002241
full.R2 <- r.squaredGLMM(lmer.full.annual)
full.fix <- full.R2[1] #.123
full.rand <- full.R2[2] - full.R2[1] #.477
...
fixed.full.lm <- lm(INCREMENT ~ PFT + DBH15 + CANOPY15, data = combined.all.diss.common)
anova.full.fix <- anova(fixed.full.lm)
summary(fixed.full.lm)

#var(predict(lm(INCREMENT ~ PFT + DBH15 + CANOPY15, data = combined.all.diss.common)))
full.inc.ss <- anova.full.fix$`Sum Sq`/sum(anova.full.fix$`Sum Sq`)
full.fix.part.vars <- cbind(rownames(anova.full.fix), anova.full.fix$Df, anova.full.fix$`F value`, anova.full.fix$`Pr(>F)`, full.inc.ss)

#matches what I've done for the full model
# var(predict(lm(INCREMENT ~ PFT, data = combined.all.diss.common)))
# var(predict(lm(INCREMENT ~ CANOPY15, data = combined.all.diss.common)))
# var(predict(lm(INCREMENT ~ DBH15, data = combined.all.diss.common)))
var(predict(lm(INCREMENT ~ DBH15 + PFT + CANOPY15, data = combined.all.diss.common)))
...
# grp.n <- c(ngrps(lmer.full.annual),69980) #total N - groups
# full.vars.n <-cbind(full.vars, grp.n)
# full.model.vars <- full.vars.n$vcov*full.vars.n$grp.n
# fullmodel.part.vars.notcorrect <- full.model.vars/sum(full.model.vars) #this doesn't work
# percent.fullmodel.part.vars.notcorrect <- fullmodel.part.vars.correct*100

# #calculating partial variances for random effects
# var.full.vars = full.vars$vcov/sum(full.vars$vcov)
# full.part.vars <- cbind(full.vars$grp,var.full.vars)
# #test
...
#Space ANNUAL Growth, only #PFT included, Canopy & DBH not included
#running to test validity of my method for combining fixed and random variances
lmer.space.annual <- lmer(INCREMENT ~ PFT + (1|SITE/PLOT/SUB/CORE_ID15) + (1|YEAR) + (1|SPP), data = combined.all.diss.common)
anova(lmer.space.annual)
summary(lmer.space.annual)

#variances
space.vars <- as.data.frame(VarCorr(lmer.space.annual, order = "cov.last")) #random effect variances
var.space.vars = space.vars$vcov/sum(space.vars$vcov)
percent.var.space <- var.space.vars*100

fixed.pft.lm <- lm(INCREMENT ~ PFT, data = combined.all.diss.common)
anova.pft.fix <- anova(fixed.pft.lm)
summary(fixed.pft.lm)



#below not used in final analysis#
####################################################################
#wood
...
#DBH
lm.size.simp <- lm(INCREMENT ~ DBH15, data = combined.all.diss.common)
summary(lm.size.simp)

lm.size.yr <- lm(INCREMENT ~ TAG + DBH15, data = combined.all.diss.common)
anova(lm.size.yr)
summary(lm.size.yr)

lm.size.avg <- lm(mean.growth.50yr ~ DBH15, data = mean.growth.fig.data)
anova(lm.size.avg)
summary(lm.size.avg)

plot(mean.growth.50yr ~ DBH15, data = mean.growth.fig.data)
abline(lm.size.avg)
...
#DBH
lm.size.simp15 <- lm(mean.growth.50yr ~ DBH15, data = mean.growth.fig.data)
summary(lm.size.simp15)

####Test with all years dbh to report R2####
lm.size.simp14 <- lm(mean.growth.50yr ~ DBH14, data = mean.growth.fig.data)
summary(lm.size.simp14)

lm.size.simp13 <- lm(mean.growth.50yr ~ DBH13, data = mean.growth.fig.data)
summary(lm.size.simp13)

lm.size.simp12 <- lm(mean.growth.50yr ~ DBH12, data = mean.growth.fig.data)
summary(lm.size.simp12)

lm.size.simp11 <- lm(mean.growth.50yr ~ DBH11, data = mean.growth.fig.data)
